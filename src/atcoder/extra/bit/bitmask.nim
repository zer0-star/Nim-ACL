when not declared ATCODER_BITUTILS_HPP:
  ...
  proc bitIndices*[B:SomeInteger](
      mask:B,
  ):seq[int] =
    result = bitsToSeq(mask)

  proc lowBitsMask*[B:SomeInteger](
      n:int,
  ):B =
    result = allSetBits[B](n)

  proc toBitString*[B:SomeInteger](
      mask:B,
      width:int = -1,
  ):string =
    result = toBitStr(mask, width)

  iterator submasks*[B:SomeInteger](
      mask:B,
  ):B =
  ...
  iterator subsetMasks*(
      positions:seq[int],
      B:typedesc[SomeInteger] = int,
  ):B =
    for subset in subsets(positions, B):
      yield subset
