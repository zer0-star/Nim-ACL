...
when not declared ATCODER_CONVEX_HULL_TRICK_ADD_MONOTONE:
  ...
  proc empty*[T](self: ConvexHullTrickAddMonotone[T]): bool =
    self.H.len == 0
  ...
  proc check[T](self: ConvexHullTrickAddMonotone[T], a, b, c: (T, T)): bool =
    ...
    return
      (b[0] - a[0]).float * self.sgn(c[1] - b[1]).float / abs(b[1] - a[1]).float >=
      (c[0] - b[0]).float * self.sgn(b[1] - a[1]).float / abs(c[1] - b[1]).float
  ...
  proc add*[T](self: var ConvexHullTrickAddMonotone[T], a, b: T) =
    ...
    if self.H[0][0] <= a:
      if self.H[0][0] == a:
        if self.H[0][1] <= b:
          return
        discard self.H.popFirst()

      while self.H.len >= 2 and self.check(line, self.H[0], self.H[1]):
        discard self.H.popFirst()
      ...
      if self.H[self.H.len - 1][0] == a:
        if self.H[self.H.len - 1][1] <= b:
          return
        discard self.H.popLast()

      while self.H.len >= 2 and self.check(self.H[self.H.len - 2], self.H[self.H.len - 1], line):
        discard self.H.popLast()
  ...
  proc query*[T](self: ConvexHullTrickAddMonotone[T], x: T): T =
    ...
    while l + 1 < r:
      ...
      if self.get_y(self.H[m], x) >= self.get_y(self.H[m + 1], x):
        l = m
      else:
        r = m
  ...
  proc query_monotone_inc*[T](self: var ConvexHullTrickAddMonotone[T], x: T): T =
    ...
    while self.H.len >= 2 and self.get_y(self.H[0], x) >= self.get_y(self.H[1], x):
      discard self.H.popFirst()
  ...
  proc query_monotone_dec*[T](self: var ConvexHullTrickAddMonotone[T], x: T): T =
    ...
    while self.H.len >= 2 and self.get_y(self.H[self.H.len - 1], x) >= self.get_y(self.H[self.H.len - 2], x):
      discard self.H.popLast()
