import atcoder/extra/structure/universal_segtree

import std/unittest
import std/sequtils
import std/random as random_lib

import "../test/utils/random.nim"

# time manager

type TimeManager = object
  v:seq[int]

proc initTimeManager(n:int):TimeManager =
  TimeManager(v:newSeqWith(n, -1))

proc action(self: var TimeManager, p:Slice[int], time:int) =
  for i in p:
    self.v[i] = time

proc prod(self: TimeManager, p:Slice[int]):int =
  result = -1
  for i in p:
    result = max(result, self.v[i])

type S = tuple[l, r, time:int]
type T = tuple[new_time:int]

proc op_ss(l, r:S):S =
  if l.l == -1: return r
  if r.l == -1: return l
  assert l.r == r.l
  return (l.l, r.r, max(l.time, r.time))

proc op_ts(l:T, r:S):S =
  if l.new_time == -1: return r
  assert r.time < l.new_time
  return (r.l, r.r, l.new_time)

proc op_tt(l, r:T):T =
  if l.new_time == -1: return r
  if r.new_time == -1: return l
  assert l.new_time > r.new_time
  return l

proc e_s():S = (-1, -1, -1)
proc e_t():T = (-1,)

test "SegtreeStressTest, NaiveTest":
  var rnd = initRand(2020)

  for n in 1..30:
    for ph in 0..<10:
      var seg0 = initSegTree[S](n, op_ss, e_s)
      var tm = initTimeManager(n)

      for i in 0..<n:
        seg0.set(i, (i, i + 1, -1))

      var now = 0

      for q in 0..<3000:
        let ty = rnd.rand(2)

        if ty == 0:
          let (l, r) = randpair(0, n)
          let res = seg0.prod(l ..< r)

          check l == res.l
          check r == res.r
          check tm.prod(l ..< r) == res.time
          check tm.prod(l ..< r) == seg0.prod(l .. ^(n - r + 1)).time

        elif ty == 1:
          let p = rnd.rand(n - 1)
          let res = seg0.get(p)

          check p == res.l
          check p + 1 == res.r
          check tm.prod(p ..< p + 1) == res.time

        elif ty == 2:
          now.inc
          let p = rnd.rand(n - 1)

          seg0.set(p, (p, p + 1, now))
          tm.action(p ..< p + 1, now)

        else:
          assert(false)

test "SegtreeStressTest, MaxRightTest":
  var rnd = initRand(2020)

  for n in 1..30:
    for ph in 0..<10:
      var seg0 = initSegTree[S](n, op_ss, e_s)
      var tm = initTimeManager(n)

      for i in 0..<n:
        seg0.set(i, (i, i + 1, -1))

      var now = 0

      for q in 0..<1000:
        let ty = rnd.rand(1)

        if ty == 0:
          let (l, r) = randpair(0, n)

          check r == seg0.max_right(l, proc(s:S):bool =
            if s.l == -1: return true

            assert s.l == l
            assert s.time == tm.prod(l ..< s.r)

            return s.r <= r
          )

        else:
          now.inc
          let p = rnd.rand(n - 1)

          seg0.set(p, (p, p + 1, now))
          tm.action(p ..< p + 1, now)

test "SegtreeStressTest, MinLeftTest":
  var rnd = initRand(2020)

  for n in 1..30:
    for ph in 0..<10:
      var seg0 = initSegTree[S](n, op_ss, e_s)
      var tm = initTimeManager(n)

      for i in 0..<n:
        seg0.set(i, (i, i + 1, -1))

      var now = 0

      for q in 0..<1000:
        let ty = rnd.rand(1)

        if ty == 0:
          let (l, r) = randpair(0, n)

          check l == seg0.min_left(r, proc(s:S):bool =
            if s.l == -1: return true

            assert s.r == r
            assert s.time == tm.prod(s.l ..< r)

            return l <= s.l
          )

        else:
          now.inc
          let p = rnd.rand(n - 1)

          seg0.set(p, (p, p + 1, now))
          tm.action(p ..< p + 1, now)

test "DualSegtreeStressTest, NaiveTest":
  var rnd = initRand(2020)

  for n in 1..30:
    for ph in 0..<10:
      var seg0 = initDualSegTree[T](n, op_tt, e_t)
      var tm = initTimeManager(n)

      for i in 0..<n:
        seg0.set(i, (-1, ))

      var now = 0

      for q in 0..<3000:
        let ty = rnd.rand(2)

        if ty == 0:
          let p = rnd.rand(n - 1)
          let res = seg0.get(p)

          check tm.prod(p ..< p + 1) == res[0]

        elif ty == 1:
          now.inc
          let (l, r) = randpair(0, n)

          seg0.apply(l ..< r, (now,))
          tm.action(l ..< r, now)

        elif ty == 2:
          now.inc
          let p = rnd.rand(n - 1)

          seg0.apply(p, (now,))
          tm.action(p ..< p + 1, now)

        else:
          assert(false)

test "LazySegtreeStressTest, NaiveTest":
  var rnd = initRand(2020)

  for n in 1..30:
    for ph in 0..<10:
      var seg0 = initLazySegTree[S, T](n, op_ss, e_s, op_ts, op_tt, e_t)
      var tm = initTimeManager(n)

      for i in 0..<n:
        seg0.set(i, (i, i + 1, -1))

      var now = 0

      for q in 0..<3000:
        let ty = rnd.rand(3)

        if ty == 0:
          let (l, r) = randpair(0, n)
          let res = seg0.prod(l ..< r)

          check l == res.l
          check r == res.r
          check tm.prod(l ..< r) == res.time

        elif ty == 1:
          let p = rnd.rand(n - 1)
          let res = seg0.get(p)

          check p == res.l
          check p + 1 == res.r
          check tm.prod(p ..< p + 1) == res.time

        elif ty == 2:
          now.inc
          let (l, r) = randpair(0, n)

          seg0.apply(l ..< r, (now,))
          tm.action(l ..< r, now)

        elif ty == 3:
          now.inc
          let p = rnd.rand(n - 1)

          seg0.apply(p, (now,))
          tm.action(p ..< p + 1, now)

        else:
          assert(false)

test "LazySegtreeStressTest, MaxRightTest":
  var rnd = initRand(2020)

  for n in 1..30:
    for ph in 0..<10:
      var seg0 = initLazySegTree[S, T](n, op_ss, e_s, op_ts, op_tt, e_t)
      var tm = initTimeManager(n)

      for i in 0..<n:
        seg0.set(i, (i, i + 1, -1))

      var now = 0

      for q in 0..<1000:
        let ty = rnd.rand(2)

        if ty == 0:
          let (l, r) = randpair(0, n)

          check r == seg0.max_right(l, proc(s:S):bool =
            if s.l == -1: return true

            assert s.l == l
            assert s.time == tm.prod(l ..< s.r)

            return s.r <= r
          )

        elif ty == 1:
          now.inc
          let (l, r) = randpair(0, n)

          seg0.apply(l ..< r, (now,))
          tm.action(l ..< r, now)

        elif ty == 2:
          now.inc
          let p = rnd.rand(n - 1)

          seg0.apply(p, (now,))
          tm.action(p ..< p + 1, now)

        else:
          assert(false)

test "LazySegtreeStressTest, MinLeftTest":
  var rnd = initRand(2020)

  for n in 1..30:
    for ph in 0..<10:
      var seg0 = initLazySegTree[S, T](n, op_ss, e_s, op_ts, op_tt, e_t)
      var tm = initTimeManager(n)

      for i in 0..<n:
        seg0.set(i, (i, i + 1, -1))

      var now = 0

      for q in 0..<1000:
        let ty = rnd.rand(2)

        if ty == 0:
          let (l, r) = randpair(0, n)

          check l == seg0.min_left(r, proc(s:S):bool =
            if s.l == -1: return true

            assert s.r == r
            assert s.time == tm.prod(s.l ..< r)

            return l <= s.l
          )

        elif ty == 1:
          now.inc
          let (l, r) = randpair(0, n)

          seg0.apply(l ..< r, (now,))
          tm.action(l ..< r, now)

        elif ty == 2:
          now.inc
          let p = rnd.rand(n - 1)

          seg0.apply(p, (now,))
          tm.action(p ..< p + 1, now)

        else:
          assert(false)
